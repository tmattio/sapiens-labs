...
module Fallback = struct
  let createElement ~status () =
    let t = Opium.Status.default_reason_phrase status ^ " · Sapiens" in
    let error = Opium.Status.default_reason_phrase status in
    let code = Opium.Status.to_code status in
    let message = Opium.Status.long_reason_phrase status in
    html
      (head
         (title (txt t))
         ...
         [ div
             ...
             [ div
                 ...
                 [ h2
                     ~a:
                       [ a_class
                           [ "text-2xl leading-8 font-semibold font-display text-gray-900 sm:text-3xl sm:leading-9" ]
                       ]
                     [ txt (string_of_int code); txt " "; txt error ]
                 ; div ~a:[ a_class [ "mt-0 mb-4 text-gray-600" ] ] [ txt message ]
                 ]
             ]
         ])
end

let fallback ~status = Fallback.createElement ~status ()
