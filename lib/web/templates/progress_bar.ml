...
let createElement ?label ~percentage ~color () =
        ...
        [ (match label with
          ...
          | Some label ->
            div
              [ span
                  ~a:
                    [ a_class [ "text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full " ^ label_class ] ]
                  [ txt label ]
              ])
        ; div
            ~a:[ a_class [ "text-right" ] ]
            [ span ~a:[ a_class [ "text-xs font-semibold inline-block " ^ score_class ] ] [ txt progress_label ] ]
        ]
    ; div
        ~a:[ a_class [ "overflow-hidden h-2 text-xs flex rounded " ^ bg_class ] ]
        [ div
            ~a:
              [ a_style ("width: " ^ string_of_int percentage ^ "%")
              ; a_class
                  [ "shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center " ^ fg_class ]
              ]
            []
        ]
    ]
